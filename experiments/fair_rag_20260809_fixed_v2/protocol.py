...
from op_rag.config import DATA_DIR as KB_DATA_DIR  # noqa: E402
from op_rag.config import FORMULA_ALIAS_MAP, HERB_ALIAS_MAP  # noqa: E402
...
# default open-source dataset. Public data inspection uses the read-only herb viewer.
PAPER_DATA_DIR = Path(
...
CASES_PATH = first_existing_path(
    PAPER_DATA_DIR / "eval_cases_unified_001_050.jsonl",
    PAPER_DATA_DIR / "unified_cases" / "eval_cases_unified_001_050.jsonl",
)
...
REFERENCES_PATH = KB_DATA_DIR / "references.json"
...
def formula_match_keys(name: str | None) -> set[str]:
    if not name:
        return set()
    compact = "".join(str(name).split())
    normalized = "".join((normalize_formula(compact) or compact).split())
    keys: set[str] = set()
...
def resolve_formula(case: dict[str, Any], kb: dict[str, Any]) -> dict[str, Any]:
    """Resolve a visible formula name without using the retired G0-G4 runner."""
    raw = case.get("reference_main_formula_name") or case.get("reference_formula_name_raw")
    if not raw:
        return {"id": None, "raw_name": None, "std_name": None, "kb_status": "unmapped"}

    target = "".join(str(raw).split())
    normalized_target = "".join(normalize_formula(target).split())
    target_keys = formula_match_keys(target)
    matches: list[tuple[int, dict[str, Any]]] = []
    for record in kb.get("formulas", []):
        name = str(record.get("name") or "").strip()
        compact_name = "".join(name.split())
        normalized_name = "".join(normalize_formula(compact_name).split())
        if target == compact_name:
            matches.append((3, record))
        elif normalized_target == normalized_name:
            matches.append((2, record))
        elif target_keys & formula_match_keys(compact_name):
            matches.append((1, record))

    if matches:
        best_score = max(score for score, _ in matches)
        best = [record for score, record in matches if score == best_score]
        if len(best) == 1:
            record = best[0]
            return {
                "id": record.get("formula_id"),
                "raw_name": raw,
...
def build_case_occurrence_provenance(
    ...
    for case in cases:
        formula_id = resolve_formula(case, kb).get("id")
        if not formula_id:
            continue
...
def build_internal_reference(
    ...
    *,
    ...
    formula_resolution = resolve_formula(case, kb)
    formula_id = formula_resolution.get("id")
    ...
    primary_consistent = (
        ...
        if primary_relation_status != RELATION_INSUFFICIENT
        else None
    )
    any_consistent = (
        relation_status == RELATION_SUPPORTED
        if relation_status != RELATION_INSUFFICIENT
        else None
    )
    ...
    return {
        ...
        "internal_case_metrics": {
            "primary_syndrome_resolved": bool(primary_syndrome_id),
            "formula_mapped": formula_resolution.get("kb_status") == "mapped",
            "formula_source_supported_leave_one_source_out": formula_evidence_available,
            "primary_formula_concordance": primary_consistent,
            "any_formula_concordance": any_consistent,
            "core_herb_annotation_coverage": core_cov,
            "reference_herb_annotation_coverage": ratio_record(
                mechanism_supported_plan_herbs, plan_herbs
            )["value"],
            "formula_composition_annotation_coverage": formula_cov,
