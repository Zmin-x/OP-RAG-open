...
def build_resources(cases: list[dict[str, Any]]) -> dict[str, Any]:
    original_kb = load_kb()
...
def case_objects(cases: list[dict[str, Any]], kb: dict[str, Any]) -> list[dict[str, Any]]:
    rows: list[dict[str, Any]] = []
    for case in cases:
        excluded = {source_document_id(case)} if case.get("source_group") != "hospital_real_case" else set()
        reference = build_internal_reference(case, kb, excluded_source_ids=excluded)
        ...
        rows.append(
            {
                ...
                "reference": reference,
                "records": records,
                "resolved_formula_id": resolve_formula(case, kb).get("id"),
            }
        )
    return rows
...
def case_error_analysis(
    ...
    for obj in objects:
        ...
        formula_id = obj.get("resolved_formula_id")
        case_rows.append(
            {
                "case_id": reference["case_id"],
                "source_group": reference.get("source_group"),
...
def main() -> None:
    ...
    kb = build_resources(cases)
    objects = case_objects(cases, kb)
