...
<body>
  <div class="wrap">
    <div class="hero">
      <h1>Herbs 数据可视化面板</h1>
      <div class="muted">
        本地浏览 `data/herbs.json` 的靶点、通路、证据文献与描述摘要。支持按药名、通路、靶点和文献检索。
      </div>
      <div class="stats">
  ...
  <script>
    ...
    document.getElementById('statTargets').textContent = herbs.reduce((n, h) => n + (h.targets_op_related?.length || 0), 0);
    document.getElementById('statPathways').textContent = herbs.reduce((n, h) => n + (h.pathways?.length || 0), 0);
    document.getElementById('statPapers').textContent = herbs.reduce((n, h) => n + (h.evidence_papers?.length || 0), 0);
    ...
    function applyFilters() {{
      ...
      filtered.sort((a, b) => {{
        if (sortBy === 'targets') return (b.targets_op_related?.length || 0) - (a.targets_op_related?.length || 0) || a.herb_name.localeCompare(b.herb_name, 'zh-Hans-CN');
        if (sortBy === 'pathways') return (b.pathways?.length || 0) - (a.pathways?.length || 0) || a.herb_name.localeCompare(b.herb_name, 'zh-Hans-CN');
        if (sortBy === 'papers') return (b.evidence_papers?.length || 0) - (a.evidence_papers?.length || 0) || a.herb_name.localeCompare(b.herb_name, 'zh-Hans-CN');
        return a.herb_name.localeCompare(b.herb_name, 'zh-Hans-CN');
      }});
      return filtered;
    }}

    let currentName = herbs[0]?.herb_name || '';
